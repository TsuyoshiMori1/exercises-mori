...
function corsMiddleware(_url, _req, res) {
  ...
  res.setHeader("Access-Control-Allow-Origin", "http://localhost:3000"); // 許可するオリジン
  res.setHeader("Access-Control-Allow-Credentials", "true"); // クロスオリジンでのCookie送信を許可
  res.setHeader(
    "Access-Control-Allow-Methods",
    "GET, POST, PATCH, DELETE, OPTIONS"
  ); // 許可するメソッド
  res.setHeader("Access-Control-Allow-Headers", "Content-Type"); // 許可するヘッダ
  return true;
}
